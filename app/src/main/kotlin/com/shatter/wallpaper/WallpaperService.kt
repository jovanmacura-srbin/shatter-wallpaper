...
class ShatterWallpaperService : WallpaperService() {
    ...
    inner class ShatterEngine : Engine() {
        ...
        private var w = 1080; private var h = 1920
        ...
        data class Shard(
            ...
            val dispX: Float, val dispY: Float,
            val rot: Float,
            val bright: Float,
            val tiltAx: Float, val tiltAy: Float,
            val litEdges: List<Pair<Float, Float>>,  // ndot per edge
            val edgePts: List<FloatArray>             // edge point pairs
        )

        // ── Build Voronoi via Sutherland-Hodgman ─────────────────────────────
        private fun buildShards(sw: Int, sh: Int) {
            rngS = 0xC0FFEE42L
            val hx = sw * 0.50f; val hy = sh * 0.36f
            val clkR = sw * 0.20f
            ...
            fun clipHalf(poly: List<FloatArray>, si: FloatArray, sj: FloatArray): List<FloatArray> {
                ...
                for (k in poly.indices) {
                    ...
                    if ((da <= 0f) != (db <= 0f)) {
                        val t = da / (da - db)
                        result.add(floatArrayOf(a[0]+t*(b[0]-a[0]), a[1]+t*(b[1]-a[1])))
                    }
                }
                return result
            }
            ...
            for (i in 0 until NS) {
                ...
                val tiltAx = rr(-0.7f, 0.7f); val tiltAy = rr(-0.7f, 0.7f)

                // Edge lighting
                val litEdges = mutableListOf<Pair<Float, Float>>()
                val edgePts = mutableListOf<FloatArray>()
                for (k in poly.indices) {
                    val p1 = poly[k]; val p2 = poly[(k+1) % poly.size]
                    val ex = p2[0]-p1[0]; val ey = p2[1]-p1[1]
                    val el = sqrt(ex*ex+ey*ey) + 1e-5f
                    val nx2 = ey/el; val ny2 = -ex/el
                    val ndot = nx2*LDX + ny2*LDY
                    litEdges.add(Pair(ndot, el))
                    edgePts.add(floatArrayOf(p1[0], p1[1], p2[0], p2[1]))
                }

                // Build path (displaced + rotated)
                val path = Path()
                val matrix = Matrix()
                ...
                // Transform edge points too
                val edgePtsTransformed = mutableListOf<FloatArray>()
                for (ep in edgePts) {
                    val t = FloatArray(4)
                    t[0]=ep[0]; t[1]=ep[1]; t[2]=ep[2]; t[3]=ep[3]
                    matrix.mapPoints(t, 0, t, 0, 2)
                    edgePtsTransformed.add(t)
                }

                shards.add(Shard(path, cx2+dispX, cy2+dispY, dispX, dispY, rot, bright, tiltAx, tiltAy, litEdges, edgePtsTransformed))
            }
            shardsBuilt = true
        }
        ...
        private fun drawFrame(canvas: Canvas) {
            ...
            for (shard in shards) {
                ...
                for (k in shard.litEdges.indices) {
                    val (ndot, _) = shard.litEdges[k]
                    val ep = shard.edgePtsTransformed[k]
                    if (ndot > 0.04f) {
                        ...
                        val alpha2 = min(0.65f, ndot * 0.55f + shimmer * 0.06f)
                        edgePaint.color = Color.argb((alpha2 * 255).toInt().coerceIn(0,255), 205, 238, 255)
            ...
            val glow = RadialGradient(hx, hy, w * 0.44f,
                intArrayOf(
                    Color.argb((0.26f + shimmer * 0.08f).times(255).toInt().coerceIn(0,255), 12, 40, 115),
                    Color.argb(26, 5, 18, 58),
                    Color.TRANSPARENT
                ),
            ...
            // Vignette
            val vigPaint = Paint(Paint.ANTI_ALIAS_FLAG)
            val vig = RadialGradient(w*0.5f, h*0.5f, h*0.9f,
        ...
        private fun drawClock(canvas: Canvas, shimmer: Float) {
            ...
            val faceShader = RadialGradient(cx - r*.20f, cy - r*.22f, r,
            ...
            for (i in 0..59) {
                ...
                val big = i % 5 == 0
                tickPaint.strokeWidth = if (big) r * 0.030f else r * 0.012f
                tickPaint.color = if (big) Color.argb(189, 118, 182, 250) else Color.argb(66, 58, 112, 195)
                val r1 = r - r*0.06f; val r2 = r1 - if (big) r*0.16f else r*0.07f
                canvas.drawLine(cx + cos(a)*r1, cy + sin(a)*r1, cx + cos(a)*r2, cy + sin(a)*r2, tickPaint)
            }
            ...
            val handPaint = Paint(Paint.ANTI_ALIAS_FLAG).apply { style = Paint.Style.STROKE; strokeCap = Paint.Cap.ROUND }

            // Hour
            val ha = (hour/12f)*PI.toFloat()*2f - PI.toFloat()/2f
            handPaint.strokeWidth = r * 0.065f; handPaint.color = Color.argb(234, 208, 232, 255)
            canvas.drawLine(cx - cos(ha)*r*.14f, cy - sin(ha)*r*.14f, cx + cos(ha)*r*.52f, cy + sin(ha)*r*.52f, handPaint)

            // Minute
            val ma = (min/60f)*PI.toFloat()*2f - PI.toFloat()/2f
            handPaint.strokeWidth = r * 0.038f; handPaint.color = Color.argb(214, 175, 218, 255)
            canvas.drawLine(cx - cos(ma)*r*.14f, cy - sin(ma)*r*.14f, cx + cos(ma)*r*.73f, cy + sin(ma)*r*.73f, handPaint)

            // Second
            val sa = (sec/60f)*PI.toFloat()*2f - PI.toFloat()/2f
            handPaint.strokeWidth = r * 0.018f; handPaint.color = Color.argb(224, 45, 158, 255)
            canvas.drawLine(cx - cos(sa)*r*.20f, cy - sin(sa)*r*.20f, cx + cos(sa)*r*.80f, cy + sin(sa)*r*.80f, handPaint)

            // Center dot
            val dotPaint = Paint(Paint.ANTI_ALIAS_FLAG).apply { color = Color.argb(242, 178, 224, 255) }
            canvas.drawCircle(cx, cy, r * 0.052f, dotPaint)
        }
